...
function AddPage(){
    const { invalidError,category } = usePage<any>().props;
    const [btnLoading, setBtnLoading] = useState(false);
    const [html, setHtml] = useState('');
    const csrfToken = useCsrfToken();

    const schema = yup.object({ 
      id:yup.number().notRequired().nonNullable(),
    ...
    const onSubmit = async (data: any) => {
      setBtnLoading(true);
      Inertia.post('/admin/blog/category/update',data,
        {
            preserveScroll: true,
            headers: {
                            ...
                            <form className="theme-form theme-form-2 mega-form" onSubmit={handleSubmit(onSubmit)} > 
                              ...
                              <div className="mb-4 row align-items-center">
                                    ...
                                    <div className="col-sm-9">
                                        ...
                                        {
                                          category?.icon && (
                                            <Image src={category?.icon} width={100}></Image>
                                          )
                                        }
                                        {errors?.icon?.message && (
